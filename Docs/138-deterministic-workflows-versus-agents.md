# 138. Deterministic workflows versus agents

**Track:** AGENT  
**Companion notebook:** [../Practice/138-deterministic-workflows-versus-agents.ipynb](../Practice/138-deterministic-workflows-versus-agents.ipynb)

## Purpose

selection criteria, reliability, and maintenance.
...
**Deterministic workflows versus agents** belongs to **Agents and workflows** and is prioritized as **P0**. Its practical scope is selection criteria, reliability, and maintenance. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Selection Criteria:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Reliability:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Maintenance:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Deterministic workflows versus agents. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Deterministic workflows versus agents when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Deterministic workflows versus agents**. In plain language: selection criteria, reliability, and maintenance. The topic belongs to the **AGENT** track, so interpret every operation through this larger foundation: An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Deterministic workflows versus agents**, keep selection criteria, reliability, and maintenance. as the concrete goal while examining every function below.
...
### `selection criteria`
A core part of Deterministic workflows versus agents; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`selection criteria`**: A core part of Deterministic workflows versus agents; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`reliability`**: A core part of Deterministic workflows versus agents; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`maintenance`**: A core part of Deterministic workflows versus agents; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Deterministic workflows versus agents**.
