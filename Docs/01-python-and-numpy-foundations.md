# 01. Python and NumPy Foundations

**Track:** DA  
**Companion notebook:** [../Practice/01-python-and-numpy-foundations.ipynb](../Practice/01-python-and-numpy-foundations.ipynb)

## Purpose

Arrays, vectorization, broadcasting, indexing, aggregation, and reproducible random sampling.
...
This lesson focuses on **Python and NumPy Foundations**. In plain language: Arrays, vectorization, broadcasting, indexing, aggregation, and reproducible random sampling. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Python and NumPy Foundations**, keep arrays, vectorization, broadcasting, indexing, aggregation, and reproducible random sampling. as the concrete goal while examining every function below.
...
### `np.array`
Creates an ndarray from Python data. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `np.random.default_rng`
Creates a reproducible modern random generator. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `reshape`
Changes dimensions without changing data when possible. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `mean/std`
Compute center and spread along a chosen axis. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `boolean indexing`
Filters elements using a Boolean mask. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`np.array`**: Creates an ndarray from Python data.
2. **`np.random.default_rng`**: Creates a reproducible modern random generator.
3. **`reshape`**: Changes dimensions without changing data when possible.
4. **`mean/std`**: Compute center and spread along a chosen axis.
5. **`boolean indexing`**: Filters elements using a Boolean mask.
...
1. Draw the complete state-transition diagram for **Python and NumPy Foundations**.
