...
**Track:** ML  
**Companion notebook:** [../Practice/11-k-nearest-neighbors.ipynb](../Practice/11-k-nearest-neighbors.ipynb)

## Purpose

Distance-based prediction, scaling, choosing k, and understanding local decision boundaries.
...
Predicts from nearby labeled samples. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `n_neighbors`
Controls neighborhood size and bias-variance tradeoff. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `weights`
Chooses uniform or distance-weighted voting. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `cross_val_score`
Evaluates across repeated train/validation folds. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `StandardScaler`
Makes distance comparisons scale-consistent. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`KNeighborsClassifier`**: Predicts from nearby labeled samples.
2. **`n_neighbors`**: Controls neighborhood size and bias-variance tradeoff.
3. **`weights`**: Chooses uniform or distance-weighted voting.
4. **`cross_val_score`**: Evaluates across repeated train/validation folds.
5. **`StandardScaler`**: Makes distance comparisons scale-consistent.
...
1. Draw the complete state-transition diagram for **K-Nearest Neighbors**.
