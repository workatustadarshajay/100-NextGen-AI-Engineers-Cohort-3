...
**Track:** RAG  
**Companion notebook:** [../Practice/26-rag-fundamentals.ipynb](../Practice/26-rag-fundamentals.ipynb)

## Purpose

Retrieval-augmented generation architecture, document loading, chunking, retrieval, context assembly, and grounded prompts.
...
This lesson focuses on **RAG Fundamentals**. In plain language: Retrieval-augmented generation architecture, document loading, chunking, retrieval, context assembly, and grounded prompts. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **RAG Fundamentals**, keep retrieval-augmented generation architecture, document loading, chunking, retrieval, context assembly, and grounded prompts. as the concrete goal while examining every function below.
...
### `document loader`
Reads source content plus metadata. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `chunking`
Splits content into retrievable units. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `retriever`
Ranks chunks for a query. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `context assembly`
Formats selected evidence for the model. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `grounded prompt`
Restricts the answer to supplied evidence. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`document loader`**: Reads source content plus metadata.
2. **`chunking`**: Splits content into retrievable units.
3. **`retriever`**: Ranks chunks for a query.
4. **`context assembly`**: Formats selected evidence for the model.
5. **`grounded prompt`**: Restricts the answer to supplied evidence.
...
1. Draw the complete state-transition diagram for **RAG Fundamentals**.
