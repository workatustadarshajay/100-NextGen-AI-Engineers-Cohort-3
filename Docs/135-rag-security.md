...
**Track:** RAG  
**Companion notebook:** [../Practice/135-rag-security.ipynb](../Practice/135-rag-security.ipynb)

## Purpose

poisoned documents, prompt injection, unauthorized retrieval, and source validation.
...
**RAG security** belongs to **RAG and search** and is prioritized as **P1**. Its practical scope is poisoned documents, prompt injection, unauthorized retrieval, and source validation. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Poisoned Documents:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Prompt Injection:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Unauthorized Retrieval:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Source Validation:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for RAG security. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered RAG security when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **RAG security**. In plain language: poisoned documents, prompt injection, unauthorized retrieval, and source validation. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **RAG security**, keep poisoned documents, prompt injection, unauthorized retrieval, and source validation. as the concrete goal while examining every function below.
...
### `poisoned documents`
A core part of RAG security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`poisoned documents`**: A core part of RAG security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`prompt injection`**: A core part of RAG security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`unauthorized retrieval`**: A core part of RAG security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`source validation`**: A core part of RAG security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **RAG security**.
