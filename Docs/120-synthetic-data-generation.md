# 120. Synthetic data generation

**Track:** LLM  
**Companion notebook:** [../Practice/120-synthetic-data-generation.ipynb](../Practice/120-synthetic-data-generation.ipynb)

## Purpose

filtering, diversity, contamination, labeling, and evaluation.
...
**Synthetic data generation** belongs to **LLM engineering** and is prioritized as **P2**. Its practical scope is filtering, diversity, contamination, labeling, and evaluation. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Filtering:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Diversity:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Contamination:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Labeling:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Evaluation:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Synthetic data generation. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Synthetic data generation when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
### `filtering`
A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`filtering`**: A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`diversity`**: A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`contamination`**: A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`labeling`**: A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`evaluation`**: A core part of Synthetic data generation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Synthetic data generation**.
