# 25. Transformers and Attention

**Track:** DL  
**Companion notebook:** [../Practice/25-transformers-and-attention.ipynb](../Practice/25-transformers-and-attention.ipynb)

## Purpose

Queries, keys, values, scaled dot-product attention, softmax, masks, and multi-head intuition.
...
### `query/key/value`
Projected token roles used to match and combine information. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `scaled dot product`
Computes compatibility while controlling magnitude. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `softmax`
Converts scores into normalized attention weights. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `mask`
Prevents attention to padding or future tokens. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `multi-head attention`
Learns several relation subspaces in parallel. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`query/key/value`**: Projected token roles used to match and combine information.
2. **`scaled dot product`**: Computes compatibility while controlling magnitude.
3. **`softmax`**: Converts scores into normalized attention weights.
4. **`mask`**: Prevents attention to padding or future tokens.
5. **`multi-head attention`**: Learns several relation subspaces in parallel.
...
1. Draw the complete state-transition diagram for **Transformers and Attention**.
