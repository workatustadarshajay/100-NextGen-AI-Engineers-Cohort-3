# 17. Model Evaluation and Tuning

**Track:** ML  
**Companion notebook:** [../Practice/17-model-evaluation-and-tuning.ipynb](../Practice/17-model-evaluation-and-tuning.ipynb)

## Purpose

Cross-validation, metrics, baselines, hyperparameter search, and avoiding test-set leakage.
...
This lesson focuses on **Model Evaluation and Tuning**. In plain language: Cross-validation, metrics, baselines, hyperparameter search, and avoiding test-set leakage. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Model Evaluation and Tuning**, keep cross-validation, metrics, baselines, hyperparameter search, and avoiding test-set leakage. as the concrete goal while examining every function below.
...
### `cross_validate`
Evaluates one or several metrics across folds. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `GridSearchCV`
Exhaustively tests parameter combinations within CV. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `RandomizedSearchCV`
Samples combinations from distributions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `scoring`
Selects the optimization metric. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `best_estimator_`
Refitted model using the selected parameters. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`cross_validate`**: Evaluates one or several metrics across folds.
2. **`GridSearchCV`**: Exhaustively tests parameter combinations within CV.
3. **`RandomizedSearchCV`**: Samples combinations from distributions.
4. **`scoring`**: Selects the optimization metric.
5. **`best_estimator_`**: Refitted model using the selected parameters.
...
1. Draw the complete state-transition diagram for **Model Evaluation and Tuning**.
