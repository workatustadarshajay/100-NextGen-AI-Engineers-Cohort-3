# 19. Neural Network Fundamentals

**Track:** DL  
**Companion notebook:** [../Practice/19-neural-network-fundamentals.ipynb](../Practice/19-neural-network-fundamentals.ipynb)

## Purpose

Perceptrons, activations, forward propagation, loss, gradients, and gradient descent using NumPy.
...
Combines inputs and learned weights. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `sigmoid`
Maps logits to values between zero and one. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `forward pass`
Computes predictions from current parameters. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `loss`
Quantifies prediction error. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `gradient descent`
Updates parameters opposite the loss gradient. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`matrix multiplication (@)`**: Combines inputs and learned weights.
2. **`sigmoid`**: Maps logits to values between zero and one.
3. **`forward pass`**: Computes predictions from current parameters.
4. **`loss`**: Quantifies prediction error.
5. **`gradient descent`**: Updates parameters opposite the loss gradient.
...
1. Draw the complete state-transition diagram for **Neural Network Fundamentals**.
