# 136. Corrective and adaptive RAG

**Track:** RAG  
**Companion notebook:** [../Practice/136-corrective-and-adaptive-rag.ipynb](../Practice/136-corrective-and-adaptive-rag.ipynb)

## Purpose

retrieval confidence, web fallback, query repair, and routing.
...
**Corrective and adaptive RAG** belongs to **RAG and search** and is prioritized as **P2**. Its practical scope is retrieval confidence, web fallback, query repair, and routing. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Retrieval Confidence:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Web Fallback:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Query Repair:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Routing:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Corrective and adaptive RAG. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Corrective and adaptive RAG when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Corrective and adaptive RAG**. In plain language: retrieval confidence, web fallback, query repair, and routing. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Corrective and adaptive RAG**, keep retrieval confidence, web fallback, query repair, and routing. as the concrete goal while examining every function below.
...
A core part of Corrective and adaptive RAG; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`retrieval confidence`**: A core part of Corrective and adaptive RAG; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`web fallback`**: A core part of Corrective and adaptive RAG; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`query repair`**: A core part of Corrective and adaptive RAG; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`routing`**: A core part of Corrective and adaptive RAG; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Corrective and adaptive RAG**.
