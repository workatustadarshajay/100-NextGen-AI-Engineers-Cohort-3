# 32. Probability and Sampling

**Track:** DA  
**Companion notebook:** [../Practice/32-probability-and-sampling.ipynb](../Practice/32-probability-and-sampling.ipynb)

## Purpose

Random variables, distributions, conditional probability, sampling error, law of large numbers, and simulation.
...
This lesson focuses on **Probability and Sampling**. In plain language: Random variables, distributions, conditional probability, sampling error, law of large numbers, and simulation. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Probability and Sampling**, keep random variables, distributions, conditional probability, sampling error, law of large numbers, and simulation. as the concrete goal while examining every function below.
...
### `default_rng`
Creates a reproducible random generator. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `binomial`
Samples success counts from Bernoulli trials. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `choice`
Samples values with or without replacement. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `cumsum`
Computes cumulative totals. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `standard error`
Measures variability of an estimator across samples. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`default_rng`**: Creates a reproducible random generator.
2. **`binomial`**: Samples success counts from Bernoulli trials.
3. **`choice`**: Samples values with or without replacement.
4. **`cumsum`**: Computes cumulative totals.
5. **`standard error`**: Measures variability of an estimator across samples.
...
1. Draw the complete state-transition diagram for **Probability and Sampling**.
