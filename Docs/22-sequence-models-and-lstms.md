# 22. Sequence Models and LSTMs

**Track:** DL  
**Companion notebook:** [../Practice/22-sequence-models-and-lstms.ipynb](../Practice/22-sequence-models-and-lstms.ipynb)

## Purpose

Ordered data, recurrent state, vanishing gradients, gates, sequence windows, and LSTM use cases.
...
### `hidden state`
Carries compressed past information forward. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `tanh`
Bounds recurrent activations. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `sequence window`
Turns a series into supervised input-target pairs. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `LSTM gates`
Control writing, forgetting, and exposing memory. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `many-to-one/many-to-many`
Describe sequence input/output arrangements. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`hidden state`**: Carries compressed past information forward.
2. **`tanh`**: Bounds recurrent activations.
3. **`sequence window`**: Turns a series into supervised input-target pairs.
4. **`LSTM gates`**: Control writing, forgetting, and exposing memory.
5. **`many-to-one/many-to-many`**: Describe sequence input/output arrangements.
...
1. Draw the complete state-transition diagram for **Sequence Models and LSTMs**.
