...
**Track:** DA  
**Companion notebook:** [../Practice/76-multiple-testing.ipynb](../Practice/76-multiple-testing.ipynb)

## Purpose

family-wise error, false discovery rate, and pre-registration.
...
**Multiple testing** belongs to **Statistics and experimentation** and is prioritized as **P1**. Its practical scope is family-wise error, false discovery rate, and pre-registration. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Family-Wise Error:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **False Discovery Rate:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Pre-Registration:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Multiple testing. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Multiple testing when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Multiple testing**. In plain language: family-wise error, false discovery rate, and pre-registration. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Multiple testing**, keep family-wise error, false discovery rate, and pre-registration. as the concrete goal while examining every function below.
...
### `family-wise error`
A core part of Multiple testing; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`family-wise error`**: A core part of Multiple testing; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`false discovery rate`**: A core part of Multiple testing; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`pre-registration`**: A core part of Multiple testing; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Multiple testing**.
