# 24. NLP Text Classification

**Track:** DL  
**Companion notebook:** [../Practice/24-nlp-text-classification.ipynb](../Practice/24-nlp-text-classification.ipynb)

## Purpose

Token patterns, TF-IDF features, sparse matrices, classifiers, and interpreting text features.
...
### `TfidfVectorizer`
Learns weighted token and n-gram features. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `fit_transform`
Learns vocabulary and returns a sparse matrix. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `ngram_range`
Controls contiguous token sequence lengths. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `get_feature_names_out`
Returns learned vocabulary labels. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `Pipeline`
Keeps vectorization and prediction together. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`TfidfVectorizer`**: Learns weighted token and n-gram features.
2. **`fit_transform`**: Learns vocabulary and returns a sparse matrix.
3. **`ngram_range`**: Controls contiguous token sequence lengths.
4. **`get_feature_names_out`**: Returns learned vocabulary labels.
5. **`Pipeline`**: Keeps vectorization and prediction together.
...
1. Draw the complete state-transition diagram for **NLP Text Classification**.
