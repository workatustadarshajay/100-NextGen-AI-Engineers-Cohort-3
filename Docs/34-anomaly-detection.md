...
**Track:** ML  
**Companion notebook:** [../Practice/34-anomaly-detection.ipynb](../Practice/34-anomaly-detection.ipynb)

## Purpose

Detecting rare unusual observations with robust statistics and Isolation Forest while controlling contamination.
...
This lesson focuses on **Anomaly Detection**. In plain language: Detecting rare unusual observations with robust statistics and Isolation Forest while controlling contamination. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Anomaly Detection**, keep detecting rare unusual observations with robust statistics and isolation forest while controlling contamination. as the concrete goal while examining every function below.
...
### `IsolationForest`
Isolates unusual observations with random trees. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `contamination`
Sets the expected anomaly fraction for thresholding. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `decision_function`
Returns anomaly-normality scores. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `median`
Provides a robust center. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `MAD`
Provides robust dispersion against extreme values. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`IsolationForest`**: Isolates unusual observations with random trees.
2. **`contamination`**: Sets the expected anomaly fraction for thresholding.
3. **`decision_function`**: Returns anomaly-normality scores.
4. **`median`**: Provides a robust center.
5. **`MAD`**: Provides robust dispersion against extreme values.
...
1. Draw the complete state-transition diagram for **Anomaly Detection**.
