# 80. Sequential experimentation

**Track:** DA  
**Companion notebook:** [../Practice/80-sequential-experimentation.ipynb](../Practice/80-sequential-experimentation.ipynb)

## Purpose

peeking risk, alpha spending, stopping rules, and always-valid inference.
...
**Sequential experimentation** belongs to **Statistics and experimentation** and is prioritized as **P1**. Its practical scope is peeking risk, alpha spending, stopping rules, and always-valid inference. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Peeking Risk:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Alpha Spending:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Stopping Rules:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Always-Valid Inference:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Sequential experimentation. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Sequential experimentation when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
### `peeking risk`
A core part of Sequential experimentation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`peeking risk`**: A core part of Sequential experimentation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`alpha spending`**: A core part of Sequential experimentation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`stopping rules`**: A core part of Sequential experimentation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`always-valid inference`**: A core part of Sequential experimentation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Sequential experimentation**.
