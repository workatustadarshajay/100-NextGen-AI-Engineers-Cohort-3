...
**Track:** ML  
**Companion notebook:** [../Practice/84-naive-bayes.ipynb](../Practice/84-naive-bayes.ipynb)

## Purpose

conditional independence, text classification, smoothing, and calibration.
...
**Naive Bayes** belongs to **Machine learning** and is prioritized as **P1**. Its practical scope is conditional independence, text classification, smoothing, and calibration. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Conditional Independence:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Text Classification:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Smoothing:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Calibration:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Naive Bayes. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Naive Bayes when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
A core part of Naive Bayes; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`conditional independence`**: A core part of Naive Bayes; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`text classification`**: A core part of Naive Bayes; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`smoothing`**: A core part of Naive Bayes; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`calibration`**: A core part of Naive Bayes; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Naive Bayes**.
