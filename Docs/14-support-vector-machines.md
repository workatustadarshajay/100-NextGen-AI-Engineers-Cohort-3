...
**Track:** ML  
**Companion notebook:** [../Practice/14-support-vector-machines.ipynb](../Practice/14-support-vector-machines.ipynb)

## Purpose

Margins, support vectors, kernels, scaling, and the C/gamma hyperparameters.
...
This lesson focuses on **Support Vector Machines**. In plain language: Margins, support vectors, kernels, scaling, and the C/gamma hyperparameters. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Support Vector Machines**, keep margins, support vectors, kernels, scaling, and the c/gamma hyperparameters. as the concrete goal while examining every function below.
...
### `SVC`
Fits a maximum-margin classifier. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `kernel`
Defines similarity in transformed feature space. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `C`
Controls regularization versus training errors. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `gamma`
Controls RBF influence radius. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `support vectors`
Training points defining the decision boundary. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`SVC`**: Fits a maximum-margin classifier.
2. **`kernel`**: Defines similarity in transformed feature space.
3. **`C`**: Controls regularization versus training errors.
4. **`gamma`**: Controls RBF influence radius.
5. **`support vectors`**: Training points defining the decision boundary.
...
1. Draw the complete state-transition diagram for **Support Vector Machines**.
