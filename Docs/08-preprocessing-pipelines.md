...
**Track:** ML  
**Companion notebook:** [../Practice/08-preprocessing-pipelines.ipynb](../Practice/08-preprocessing-pipelines.ipynb)
...
The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Preprocessing Pipelines**, keep preventing leakage with train/test splits, imputers, scalers, encoders, column transformers, and pipelines. as the concrete goal while examining every function below.
...
### `train_test_split`
Creates independent training and test partitions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `SimpleImputer`
Learns replacement values from training data. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `StandardScaler`
Centers and scales numeric features. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `OneHotEncoder`
Encodes nominal categories safely. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `Pipeline/ColumnTransformer`
Chains transformations by column and prevents leakage. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. Draw the complete state-transition diagram for **Preprocessing Pipelines**.
