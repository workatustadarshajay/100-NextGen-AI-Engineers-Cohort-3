# 30. Files JSON and REST APIs

**Track:** PYTHON  
**Companion notebook:** [../Practice/30-files-json-and-rest-apis.ipynb](../Practice/30-files-json-and-rest-apis.ipynb)

## Purpose

Safe file handling, JSON serialization, HTTP concepts, timeouts, status codes, and resilient API parsing.
...
This lesson focuses on **Files JSON and REST APIs**. In plain language: Safe file handling, JSON serialization, HTTP concepts, timeouts, status codes, and resilient API parsing. The topic belongs to the **PYTHON** track, so interpret every operation through this larger foundation: Python engineering turns working statements into reliable components. Values have types, functions define contracts, classes combine state with behavior, exceptions represent failures, and tests preserve intended behavior while code changes.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Files JSON and REST APIs**, keep safe file handling, json serialization, http concepts, timeouts, status codes, and resilient api parsing. as the concrete goal while examining every function below.
...
### `Path.read_text`
Reads text with an explicit encoding. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `Path.write_text`
Writes text and returns character count. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `json.dumps`
Serializes Python values to JSON text. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `json.loads`
Parses JSON text into Python values. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `urlencode`
Safely encodes query parameters. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`Path.read_text`**: Reads text with an explicit encoding.
2. **`Path.write_text`**: Writes text and returns character count.
3. **`json.dumps`**: Serializes Python values to JSON text.
4. **`json.loads`**: Parses JSON text into Python values.
5. **`urlencode`**: Safely encodes query parameters.
...
1. Draw the complete state-transition diagram for **Files JSON and REST APIs**.
