# 43. Transfer Learning and Fine Tuning

**Track:** DL  
**Companion notebook:** [../Practice/43-transfer-learning-and-fine-tuning.ipynb](../Practice/43-transfer-learning-and-fine-tuning.ipynb)

## Purpose

Reusing pretrained representations, freezing layers, adapting task heads, learning-rate choices, and avoiding catastrophic forgetting.
...
### `pretrained backbone`
Provides reusable representations learned on a source task. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `freeze`
Prevents selected weights from updating. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `task head`
Maps reusable features to the new output. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `fine-tuning`
Updates some or all pretrained weights on target data. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `catastrophic forgetting`
Losing useful pretrained behavior during adaptation. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`pretrained backbone`**: Provides reusable representations learned on a source task.
2. **`freeze`**: Prevents selected weights from updating.
3. **`task head`**: Maps reusable features to the new output.
4. **`fine-tuning`**: Updates some or all pretrained weights on target data.
5. **`catastrophic forgetting`**: Losing useful pretrained behavior during adaptation.
...
1. Draw the complete state-transition diagram for **Transfer Learning and Fine Tuning**.
