# 88. Cost-sensitive learning

**Track:** ML  
**Companion notebook:** [../Practice/88-cost-sensitive-learning.ipynb](../Practice/88-cost-sensitive-learning.ipynb)

## Purpose

decision costs, weights, thresholds, and expected utility.
...
**Cost-sensitive learning** belongs to **Machine learning** and is prioritized as **P1**. Its practical scope is decision costs, weights, thresholds, and expected utility. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Decision Costs:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Weights:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Thresholds:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Expected Utility:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Cost-sensitive learning. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Cost-sensitive learning when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
### `decision costs`
A core part of Cost-sensitive learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`decision costs`**: A core part of Cost-sensitive learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`weights`**: A core part of Cost-sensitive learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`thresholds`**: A core part of Cost-sensitive learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`expected utility`**: A core part of Cost-sensitive learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Cost-sensitive learning**.
