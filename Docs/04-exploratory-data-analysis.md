...
**Track:** DA  
**Companion notebook:** [../Practice/04-exploratory-data-analysis.ipynb](../Practice/04-exploratory-data-analysis.ipynb)

## Purpose

Understanding shape, distributions, segments, correlations, and possible data-quality issues.
...
This lesson focuses on **Exploratory Data Analysis**. In plain language: Understanding shape, distributions, segments, correlations, and possible data-quality issues. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Exploratory Data Analysis**, keep understanding shape, distributions, segments, correlations, and possible data-quality issues. as the concrete goal while examining every function below.
...
### `info`
Summarizes columns, non-null counts, and dtypes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `describe`
Computes descriptive statistics. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `value_counts`
Counts category frequencies. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `corr`
Computes pairwise numeric correlation. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `skew`
Measures distribution asymmetry. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`info`**: Summarizes columns, non-null counts, and dtypes.
2. **`describe`**: Computes descriptive statistics.
3. **`value_counts`**: Counts category frequencies.
4. **`corr`**: Computes pairwise numeric correlation.
5. **`skew`**: Measures distribution asymmetry.
...
1. Draw the complete state-transition diagram for **Exploratory Data Analysis**.
