...
**Track:** ML  
**Companion notebook:** [../Practice/09-linear-regression.ipynb](../Practice/09-linear-regression.ipynb)

## Purpose

Fitting continuous targets, interpreting coefficients, checking residuals, and measuring error.
...
This lesson focuses on **Linear Regression**. In plain language: Fitting continuous targets, interpreting coefficients, checking residuals, and measuring error. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Linear Regression**, keep fitting continuous targets, interpreting coefficients, checking residuals, and measuring error. as the concrete goal while examining every function below.
...
Estimates coefficients by ordinary least squares. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `predict`
Computes target estimates for new rows. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `mean_absolute_error`
Average absolute prediction error. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `mean_squared_error`
Average squared error; penalizes large misses. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `r2_score`
Fraction of target variance explained by the model. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`LinearRegression.fit`**: Estimates coefficients by ordinary least squares.
2. **`predict`**: Computes target estimates for new rows.
3. **`mean_absolute_error`**: Average absolute prediction error.
4. **`mean_squared_error`**: Average squared error; penalizes large misses.
5. **`r2_score`**: Fraction of target variance explained by the model.
...
1. Draw the complete state-transition diagram for **Linear Regression**.
