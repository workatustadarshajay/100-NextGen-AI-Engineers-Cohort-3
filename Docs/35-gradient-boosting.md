...
**Track:** ML  
**Companion notebook:** [../Practice/35-gradient-boosting.ipynb](../Practice/35-gradient-boosting.ipynb)

## Purpose

Sequentially correcting residual errors with boosted trees, shrinkage, depth control, and staged evaluation.
...
Fits efficient histogram-based boosted trees. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `learning_rate`
Shrinks each tree contribution. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `max_iter`
Controls boosting rounds. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `max_leaf_nodes`
Limits individual learner complexity. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `l2_regularization`
Penalizes extreme leaf predictions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`HistGradientBoostingClassifier`**: Fits efficient histogram-based boosted trees.
2. **`learning_rate`**: Shrinks each tree contribution.
3. **`max_iter`**: Controls boosting rounds.
4. **`max_leaf_nodes`**: Limits individual learner complexity.
5. **`l2_regularization`**: Penalizes extreme leaf predictions.
...
1. Draw the complete state-transition diagram for **Gradient Boosting**.
