# 28. End-to-End RAG and Evaluation

**Track:** RAG  
**Companion notebook:** [../Practice/28-end-to-end-rag-and-evaluation.ipynb](../Practice/28-end-to-end-rag-and-evaluation.ipynb)

## Purpose

A complete local RAG flow plus retrieval metrics, answer faithfulness checks, citations, and failure analysis.
...
This lesson focuses on **End-to-End RAG and Evaluation**. In plain language: A complete local RAG flow plus retrieval metrics, answer faithfulness checks, citations, and failure analysis. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **End-to-End RAG and Evaluation**, keep a complete local rag flow plus retrieval metrics, answer faithfulness checks, citations, and failure analysis. as the concrete goal while examining every function below.
...
### `RAG pipeline`
Connects ingestion, retrieval, prompting, generation, and evaluation. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `hit rate@k`
Fraction of questions retrieving relevant evidence. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `MRR`
Rewards the rank of the first relevant result. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `faithfulness`
Checks whether claims are supported by context. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `citation`
Links answer claims to source identifiers. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`RAG pipeline`**: Connects ingestion, retrieval, prompting, generation, and evaluation.
2. **`hit rate@k`**: Fraction of questions retrieving relevant evidence.
3. **`MRR`**: Rewards the rank of the first relevant result.
4. **`faithfulness`**: Checks whether claims are supported by context.
5. **`citation`**: Links answer claims to source identifiers.
...
1. Draw the complete state-transition diagram for **End-to-End RAG and Evaluation**.
