...
**Track:** AGENT  
**Companion notebook:** [../Practice/142-agent-evaluation.ipynb](../Practice/142-agent-evaluation.ipynb)

## Purpose

scenario suites, tool accuracy, completion, steps, cost, and safety.
...
**Agent evaluation** belongs to **Agents and workflows** and is prioritized as **P1**. Its practical scope is scenario suites, tool accuracy, completion, steps, cost, and safety. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Scenario Suites:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Tool Accuracy:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Completion:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Steps:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Cost:** Define it, locate it in the system flow, and identify one measurable behavior.
6. **Safety:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Agent evaluation. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Agent evaluation when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.

## Before you begin

**Recommended prerequisites:** LLM structured output, Python functions, JSON schemas, API safety, and the difference between planning and execution.

**Track foundation:** An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

**Lifecycle:** `goal -> plan/proposed action -> schema validation -> authorization -> tool execution -> observation -> termination or next bounded step`

**Evidence expected:** Task completion on fixed scenarios, tool-selection accuracy, invalid-action rejection, step and cost limits, audit logs, and red-team tests.
...
This lesson focuses on **Agent evaluation**. In plain language: scenario suites, tool accuracy, completion, steps, cost, and safety. The topic belongs to the **AGENT** track, so interpret every operation through this larger foundation: An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Agent evaluation**, keep scenario suites, tool accuracy, completion, steps, cost, and safety. as the concrete goal while examining every function below.
...
7. **Operation in context:** Place the feature inside the larger AGENT lifecycle.
...
The model proposes a structured action rather than executing arbitrary text. A controller validates the action against a tool schema, checks identity and authorization, enforces budgets and step limits, executes in an isolated environment, and returns a bounded observation. The loop terminates on completion, rejection, timeout, repeated failure, or a maximum step count. Audit records connect every proposal, approval, execution, and result.
...
Use a fixed workflow when steps are known, a router when one of several deterministic handlers is sufficient, retrieval when the task is read-only knowledge access, and an agent only when dynamic multi-step tool selection is necessary. Human approval should gate financial, external communication, deletion, permission, or irreversible operations.
...
An incident assistant may search runbooks and query read-only metrics automatically, but restarting a service requires validated parameters, environment restrictions, change authorization, and human approval. Tool outputs must be size-limited and treated as untrusted. After repeated failed diagnosis, the loop should stop and escalate with its complete audit trail.
...
### `scenario suites`
A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`scenario suites`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`tool accuracy`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`completion`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`steps`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`cost`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
6. **`safety`**: A core part of Agent evaluation; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Agent evaluation**.
...
Add tools one at a time, require approval for consequential actions, isolate credentials, and prefer deterministic workflows when planning is unnecessary.
