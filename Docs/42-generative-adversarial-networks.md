...
**Track:** DL  
**Companion notebook:** [../Practice/42-generative-adversarial-networks.ipynb](../Practice/42-generative-adversarial-networks.ipynb)

## Purpose

Understanding generator-discriminator competition, adversarial objectives, alternating updates, instability, and mode collapse.
...
### `generator`
Maps random latent noise to synthetic samples. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `discriminator`
Estimates whether samples are real or generated. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `adversarial loss`
Creates competing objectives for two networks. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `alternating updates`
Optimizes discriminator and generator in turns. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `mode collapse`
Generator produces limited varieties despite diverse data. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`generator`**: Maps random latent noise to synthetic samples.
2. **`discriminator`**: Estimates whether samples are real or generated.
3. **`adversarial loss`**: Creates competing objectives for two networks.
4. **`alternating updates`**: Optimizes discriminator and generator in turns.
5. **`mode collapse`**: Generator produces limited varieties despite diverse data.
...
1. Draw the complete state-transition diagram for **Generative Adversarial Networks**.
