# 47. Reranking and Hybrid Search

**Track:** RAG  
**Companion notebook:** [../Practice/47-reranking-and-hybrid-search.ipynb](../Practice/47-reranking-and-hybrid-search.ipynb)

## Purpose

Combining lexical and vector retrieval, score normalization, reciprocal-rank fusion, and second-stage reranking.
...
This lesson focuses on **Reranking and Hybrid Search**. In plain language: Combining lexical and vector retrieval, score normalization, reciprocal-rank fusion, and second-stage reranking. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Reranking and Hybrid Search**, keep combining lexical and vector retrieval, score normalization, reciprocal-rank fusion, and second-stage reranking. as the concrete goal while examining every function below.
...
### `BM25`
Lexically ranks using term frequency and rarity. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `dense retrieval`
Ranks by embedding similarity. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `score normalization`
Makes heterogeneous scores more comparable. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `reciprocal-rank fusion`
Combines rankings without requiring score calibration. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `cross-encoder reranker`
Jointly scores query-document pairs at higher cost. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`BM25`**: Lexically ranks using term frequency and rarity.
2. **`dense retrieval`**: Ranks by embedding similarity.
3. **`score normalization`**: Makes heterogeneous scores more comparable.
4. **`reciprocal-rank fusion`**: Combines rankings without requiring score calibration.
5. **`cross-encoder reranker`**: Jointly scores query-document pairs at higher cost.
...
1. Draw the complete state-transition diagram for **Reranking and Hybrid Search**.
