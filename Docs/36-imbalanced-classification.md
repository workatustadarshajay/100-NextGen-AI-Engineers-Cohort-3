...
**Track:** ML  
**Companion notebook:** [../Practice/36-imbalanced-classification.ipynb](../Practice/36-imbalanced-classification.ipynb)

## Purpose

Class imbalance, stratification, class weights, threshold selection, precision-recall curves, and cost-aware metrics.
...
This lesson focuses on **Imbalanced Classification**. In plain language: Class imbalance, stratification, class weights, threshold selection, precision-recall curves, and cost-aware metrics. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Imbalanced Classification**, keep class imbalance, stratification, class weights, threshold selection, precision-recall curves, and cost-aware metrics. as the concrete goal while examining every function below.
...
### `class_weight`
Changes training penalty by class. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stratify`
Preserves class ratios while splitting. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `average_precision_score`
Summarizes precision-recall ranking quality. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `threshold`
Converts probabilities into decisions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `classification_report`
Reports class-specific precision, recall, and F1. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`class_weight`**: Changes training penalty by class.
2. **`stratify`**: Preserves class ratios while splitting.
3. **`average_precision_score`**: Summarizes precision-recall ranking quality.
4. **`threshold`**: Converts probabilities into decisions.
5. **`classification_report`**: Reports class-specific precision, recall, and F1.
...
1. Draw the complete state-transition diagram for **Imbalanced Classification**.
