# 146. Durable agent workflows

**Track:** AGENT  
**Companion notebook:** [../Practice/146-durable-agent-workflows.ipynb](../Practice/146-durable-agent-workflows.ipynb)

## Purpose

checkpoints, retries, compensation, idempotency, and recovery.
...
**Durable agent workflows** belongs to **Agents and workflows** and is prioritized as **P2**. Its practical scope is checkpoints, retries, compensation, idempotency, and recovery. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Checkpoints:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Retries:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Compensation:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Idempotency:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Recovery:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Durable agent workflows. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Durable agent workflows when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Durable agent workflows**. In plain language: checkpoints, retries, compensation, idempotency, and recovery. The topic belongs to the **AGENT** track, so interpret every operation through this larger foundation: An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Durable agent workflows**, keep checkpoints, retries, compensation, idempotency, and recovery. as the concrete goal while examining every function below.
...
### `checkpoints`
A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`checkpoints`**: A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`retries`**: A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`compensation`**: A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`idempotency`**: A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`recovery`**: A core part of Durable agent workflows; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Durable agent workflows**.
