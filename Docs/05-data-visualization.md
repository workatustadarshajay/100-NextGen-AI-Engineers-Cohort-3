...
**Track:** DA  
**Companion notebook:** [../Practice/05-data-visualization.ipynb](../Practice/05-data-visualization.ipynb)

## Purpose

Choosing and building plots for distributions, relationships, categories, and time trends.
...
This lesson focuses on **Data Visualization**. In plain language: Choosing and building plots for distributions, relationships, categories, and time trends. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Data Visualization**, keep choosing and building plots for distributions, relationships, categories, and time trends. as the concrete goal while examining every function below.
...
### `plt.subplots`
Creates a figure and one or more axes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `hist`
Shows a numeric distribution using bins. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `scatter`
Shows association between two numeric variables. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `bar/barh`
Compares magnitudes across categories. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `tight_layout`
Reduces label and axes overlap. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`plt.subplots`**: Creates a figure and one or more axes.
2. **`hist`**: Shows a numeric distribution using bins.
3. **`scatter`**: Shows association between two numeric variables.
4. **`bar/barh`**: Compares magnitudes across categories.
5. **`tight_layout`**: Reduces label and axes overlap.
...
1. Draw the complete state-transition diagram for **Data Visualization**.
