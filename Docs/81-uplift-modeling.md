...
**Track:** DA  
**Companion notebook:** [../Practice/81-uplift-modeling.ipynb](../Practice/81-uplift-modeling.ipynb)

## Purpose

treatment-effect heterogeneity and targeting policies.
...
**Uplift modeling** belongs to **Statistics and experimentation** and is prioritized as **P2**. Its practical scope is treatment-effect heterogeneity and targeting policies. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Treatment-Effect Heterogeneity:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Targeting Policies:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Uplift modeling. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Uplift modeling when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
A core part of Uplift modeling; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`treatment-effect heterogeneity`**: A core part of Uplift modeling; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`targeting policies`**: A core part of Uplift modeling; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Uplift modeling**.
