# 51. Tool Using Agents

**Track:** AGENT  
**Companion notebook:** [../Practice/51-tool-using-agents.ipynb](../Practice/51-tool-using-agents.ipynb)

## Purpose

Designing bounded tool loops, schemas, routing, observation handling, termination, and protection from prompt injection.
...
This lesson focuses on **Tool Using Agents**. In plain language: Designing bounded tool loops, schemas, routing, observation handling, termination, and protection from prompt injection. The topic belongs to the **AGENT** track, so interpret every operation through this larger foundation: An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Tool Using Agents**, keep designing bounded tool loops, schemas, routing, observation handling, termination, and protection from prompt injection. as the concrete goal while examining every function below.
...
### `tool schema`
Defines tool name, purpose, and typed arguments. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `router`
Selects a tool from allowed capabilities. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `observation`
Returns constrained tool results to the reasoning loop. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `step limit`
Prevents unbounded loops and cost. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `least privilege`
Exposes only the minimum required tool authority. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`tool schema`**: Defines tool name, purpose, and typed arguments.
2. **`router`**: Selects a tool from allowed capabilities.
3. **`observation`**: Returns constrained tool results to the reasoning loop.
4. **`step limit`**: Prevents unbounded loops and cost.
5. **`least privilege`**: Exposes only the minimum required tool authority.
...
1. Draw the complete state-transition diagram for **Tool Using Agents**.
