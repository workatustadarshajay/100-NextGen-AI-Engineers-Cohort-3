# 72. Causal inference fundamentals

**Track:** DA  
**Companion notebook:** [../Practice/72-causal-inference-fundamentals.ipynb](../Practice/72-causal-inference-fundamentals.ipynb)

## Purpose

confounding, DAGs, matching, difference-in-differences, and assumptions.
...
**Causal inference fundamentals** belongs to **Data engineering and analytics** and is prioritized as **P2**. Its practical scope is confounding, DAGs, matching, difference-in-differences, and assumptions. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Confounding:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **DAGs:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Matching:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Difference-In-Differences:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Assumptions:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Causal inference fundamentals. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Causal inference fundamentals when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Causal inference fundamentals**. In plain language: confounding, DAGs, matching, difference-in-differences, and assumptions. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Causal inference fundamentals**, keep confounding, dags, matching, difference-in-differences, and assumptions. as the concrete goal while examining every function below.
...
### `confounding`
A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`confounding`**: A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`DAGs`**: A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`matching`**: A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`difference-in-differences`**: A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`assumptions`**: A core part of Causal inference fundamentals; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Causal inference fundamentals**.
