...
**Track:** ML  
**Companion notebook:** [../Practice/12-decision-trees.ipynb](../Practice/12-decision-trees.ipynb)

## Purpose

Recursive splits, impurity, depth control, overfitting, and feature importance.
...
Builds axis-aligned rules recursively. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `max_depth`
Limits tree complexity. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `min_samples_leaf`
Requires support at terminal nodes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `feature_importances_`
Reports normalized impurity reduction. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `export_text`
Prints learned rules in readable form. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`DecisionTreeClassifier`**: Builds axis-aligned rules recursively.
2. **`max_depth`**: Limits tree complexity.
3. **`min_samples_leaf`**: Requires support at terminal nodes.
4. **`feature_importances_`**: Reports normalized impurity reduction.
5. **`export_text`**: Prints learned rules in readable form.
...
1. Draw the complete state-transition diagram for **Decision Trees**.
