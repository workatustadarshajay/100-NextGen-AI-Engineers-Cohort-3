# 45. LLM Tokens Sampling and Decoding

**Track:** LLM  
**Companion notebook:** [../Practice/45-llm-tokens-sampling-and-decoding.ipynb](../Practice/45-llm-tokens-sampling-and-decoding.ipynb)

## Purpose

Tokenization intuition, logits, softmax temperature, top-k, top-p, greedy decoding, and reproducibility.
...
This lesson focuses on **LLM Tokens Sampling and Decoding**. In plain language: Tokenization intuition, logits, softmax temperature, top-k, top-p, greedy decoding, and reproducibility. The topic belongs to the **LLM** track, so interpret every operation through this larger foundation: An LLM predicts token sequences from context; it does not provide a database guarantee or inherent truth check. Reliable applications constrain instructions, validate outputs, supply evidence, evaluate fixed cases, and treat model text as untrusted input.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **LLM Tokens Sampling and Decoding**, keep tokenization intuition, logits, softmax temperature, top-k, top-p, greedy decoding, and reproducibility. as the concrete goal while examining every function below.
...
### `logit`
Unnormalized model score for a candidate token. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `temperature`
Controls probability sharpness. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `top-k`
Restricts sampling to k highest-scoring tokens. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `top-p`
Restricts sampling to a probability-mass nucleus. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `greedy decoding`
Always chooses the highest-scoring next token. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`logit`**: Unnormalized model score for a candidate token.
2. **`temperature`**: Controls probability sharpness.
3. **`top-k`**: Restricts sampling to k highest-scoring tokens.
4. **`top-p`**: Restricts sampling to a probability-mass nucleus.
5. **`greedy decoding`**: Always chooses the highest-scoring next token.
...
1. Draw the complete state-transition diagram for **LLM Tokens Sampling and Decoding**.
