# 13. Random Forests and Ensembles

**Track:** ML  
**Companion notebook:** [../Practice/13-random-forests-and-ensembles.ipynb](../Practice/13-random-forests-and-ensembles.ipynb)

## Purpose

Bagging trees, reducing variance, out-of-bag evaluation, and permutation importance.
...
This lesson focuses on **Random Forests and Ensembles**. In plain language: Bagging trees, reducing variance, out-of-bag evaluation, and permutation importance. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Random Forests and Ensembles**, keep bagging trees, reducing variance, out-of-bag evaluation, and permutation importance. as the concrete goal while examining every function below.
...
Averages randomized decision trees. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `n_estimators`
Sets the number of trees. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `max_features`
Randomizes candidate features per split. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `oob_score`
Evaluates using samples omitted from each bootstrap. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `permutation_importance`
Measures score loss after shuffling a feature. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`RandomForestClassifier`**: Averages randomized decision trees.
2. **`n_estimators`**: Sets the number of trees.
3. **`max_features`**: Randomizes candidate features per split.
4. **`oob_score`**: Evaluates using samples omitted from each bootstrap.
5. **`permutation_importance`**: Measures score loss after shuffling a feature.
...
1. Draw the complete state-transition diagram for **Random Forests and Ensembles**.
