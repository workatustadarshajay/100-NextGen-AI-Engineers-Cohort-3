# 20. Dense Neural Networks

**Track:** DL  
**Companion notebook:** [../Practice/20-dense-neural-networks.ipynb](../Practice/20-dense-neural-networks.ipynb)

## Purpose

Hidden layers, nonlinear activations, regularization, optimization, and convergence with an MLP.
...
### `MLPClassifier`
Trains a feed-forward multilayer perceptron. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `hidden_layer_sizes`
Defines hidden layer widths. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `activation`
Adds nonlinear representational capacity. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `early_stopping`
Stops when validation improvement stalls. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `coefs_/loss_curve_`
Expose learned weights and training history. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`MLPClassifier`**: Trains a feed-forward multilayer perceptron.
2. **`hidden_layer_sizes`**: Defines hidden layer widths.
3. **`activation`**: Adds nonlinear representational capacity.
4. **`early_stopping`**: Stops when validation improvement stalls.
5. **`coefs_/loss_curve_`**: Expose learned weights and training history.
...
1. Draw the complete state-transition diagram for **Dense Neural Networks**.
