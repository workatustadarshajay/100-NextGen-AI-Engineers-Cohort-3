...
**Track:** ML  
**Companion notebook:** [../Practice/16-principal-component-analysis.ipynb](../Practice/16-principal-component-analysis.ipynb)

## Purpose

Standardizing correlated features, dimensionality reduction, explained variance, and reconstruction.
...
This lesson focuses on **Principal Component Analysis**. In plain language: Standardizing correlated features, dimensionality reduction, explained variance, and reconstruction. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Principal Component Analysis**, keep standardizing correlated features, dimensionality reduction, explained variance, and reconstruction. as the concrete goal while examining every function below.
...
### `PCA`
Finds orthogonal maximum-variance directions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `fit`
Learns components from training data. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `transform`
Projects observations onto components. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `explained_variance_ratio_`
Reports variance retained by each component. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `inverse_transform`
Approximately reconstructs original features. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`PCA`**: Finds orthogonal maximum-variance directions.
2. **`fit`**: Learns components from training data.
3. **`transform`**: Projects observations onto components.
4. **`explained_variance_ratio_`**: Reports variance retained by each component.
5. **`inverse_transform`**: Approximately reconstructs original features.
...
1. Draw the complete state-transition diagram for **Principal Component Analysis**.
