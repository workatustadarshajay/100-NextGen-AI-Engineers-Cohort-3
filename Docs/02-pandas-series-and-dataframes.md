...
**Track:** DA  
**Companion notebook:** [../Practice/02-pandas-series-and-dataframes.ipynb](../Practice/02-pandas-series-and-dataframes.ipynb)

## Purpose

Creating tabular data, selecting rows and columns, sorting, grouping, and joining.
...
This lesson focuses on **Pandas Series and DataFrames**. In plain language: Creating tabular data, selecting rows and columns, sorting, grouping, and joining. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Pandas Series and DataFrames**, keep creating tabular data, selecting rows and columns, sorting, grouping, and joining. as the concrete goal while examining every function below.
...
### `pd.DataFrame`
Builds a labeled two-dimensional table. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `loc/iloc`
Select by labels or integer positions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `groupby`
Implements split-apply-combine analysis. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `agg`
Applies one or more summary operations. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `merge`
Combines tables using relational keys. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`pd.DataFrame`**: Builds a labeled two-dimensional table.
2. **`loc/iloc`**: Select by labels or integer positions.
3. **`groupby`**: Implements split-apply-combine analysis.
4. **`agg`**: Applies one or more summary operations.
5. **`merge`**: Combines tables using relational keys.
...
1. Draw the complete state-transition diagram for **Pandas Series and DataFrames**.
