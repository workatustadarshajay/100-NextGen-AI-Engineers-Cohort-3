...
**Track:** ML  
**Companion notebook:** [../Practice/15-clustering-with-k-means.ipynb](../Practice/15-clustering-with-k-means.ipynb)

## Purpose

Unsupervised segmentation, inertia, initialization, choosing k, and silhouette score.
...
This lesson focuses on **Clustering with K-Means**. In plain language: Unsupervised segmentation, inertia, initialization, choosing k, and silhouette score. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Clustering with K-Means**, keep unsupervised segmentation, inertia, initialization, choosing k, and silhouette score. as the concrete goal while examining every function below.
...
### `KMeans`
Alternates assignment and centroid-update steps. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `fit_predict`
Fits clusters and returns labels. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `n_init`
Runs multiple initializations to avoid poor local solutions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `inertia_`
Within-cluster squared-distance total. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `silhouette_score`
Compares cohesion with separation; higher is better. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`KMeans`**: Alternates assignment and centroid-update steps.
2. **`fit_predict`**: Fits clusters and returns labels.
3. **`n_init`**: Runs multiple initializations to avoid poor local solutions.
4. **`inertia_`**: Within-cluster squared-distance total.
5. **`silhouette_score`**: Compares cohesion with separation; higher is better.
...
1. Draw the complete state-transition diagram for **Clustering with K-Means**.
