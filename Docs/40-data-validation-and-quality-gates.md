# 40. Data Validation and Quality Gates

**Track:** MLOPS  
**Companion notebook:** [../Practice/40-data-validation-and-quality-gates.ipynb](../Practice/40-data-validation-and-quality-gates.ipynb)

## Purpose

Defining schemas, validating ranges and categories, detecting drift, and failing pipelines with actionable messages.
...
This lesson focuses on **Data Validation and Quality Gates**. In plain language: Defining schemas, validating ranges and categories, detecting drift, and failing pipelines with actionable messages. The topic belongs to the **MLOPS** track, so interpret every operation through this larger foundation: MLOps makes data and AI behavior repeatable after experimentation. It joins versioned data, code, configuration, artifacts, deployment, observability, governance, and incident response into one controlled lifecycle.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Data Validation and Quality Gates**, keep defining schemas, validating ranges and categories, detecting drift, and failing pipelines with actionable messages. as the concrete goal while examining every function below.
...
### `required columns`
Define fields that must exist. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `between`
Checks inclusive numeric bounds. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `isin`
Checks an allowed categorical vocabulary. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `raise ValueError`
Stops invalid data with a clear reason. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `distribution shift`
Compares current and reference feature behavior. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`required columns`**: Define fields that must exist.
2. **`between`**: Checks inclusive numeric bounds.
3. **`isin`**: Checks an allowed categorical vocabulary.
4. **`raise ValueError`**: Stops invalid data with a clear reason.
5. **`distribution shift`**: Compares current and reference feature behavior.
...
1. Draw the complete state-transition diagram for **Data Validation and Quality Gates**.
