# 48. Query Transformation and Multi Query RAG

**Track:** RAG  
**Companion notebook:** [../Practice/48-query-transformation-and-multi-query-rag.ipynb](../Practice/48-query-transformation-and-multi-query-rag.ipynb)

## Purpose

Rewriting ambiguous queries, expansion, decomposition, hypothetical documents, fusion, and safeguarding user intent.
...
This lesson focuses on **Query Transformation and Multi Query RAG**. In plain language: Rewriting ambiguous queries, expansion, decomposition, hypothetical documents, fusion, and safeguarding user intent. The topic belongs to the **RAG** track, so interpret every operation through this larger foundation: RAG separates knowledge access from language generation. Ingestion creates retrievable evidence; retrieval selects context; generation synthesizes an answer; citations and evaluation test whether the answer remains grounded.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Query Transformation and Multi Query RAG**, keep rewriting ambiguous queries, expansion, decomposition, hypothetical documents, fusion, and safeguarding user intent. as the concrete goal while examining every function below.
...
### `query rewriting`
Clarifies wording while preserving intent. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `query expansion`
Adds related terms to improve recall. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `decomposition`
Splits a multi-part question into retrievable subquestions. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `multi-query retrieval`
Searches several formulations and fuses results. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `HyDE`
Retrieves using an embedding of a hypothetical answer document. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`query rewriting`**: Clarifies wording while preserving intent.
2. **`query expansion`**: Adds related terms to improve recall.
3. **`decomposition`**: Splits a multi-part question into retrievable subquestions.
4. **`multi-query retrieval`**: Searches several formulations and fuses results.
5. **`HyDE`**: Retrieves using an embedding of a hypothetical answer document.
...
1. Draw the complete state-transition diagram for **Query Transformation and Multi Query RAG**.
