...
**Track:** ML  
**Companion notebook:** [../Practice/92-active-learning.ipynb](../Practice/92-active-learning.ipynb)

## Purpose

query strategies, annotation cost, uncertainty, and sampling bias.
...
**Active learning** belongs to **Machine learning** and is prioritized as **P2**. Its practical scope is query strategies, annotation cost, uncertainty, and sampling bias. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Query Strategies:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Annotation Cost:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Uncertainty:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Sampling Bias:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Active learning. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Active learning when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Active learning**. In plain language: query strategies, annotation cost, uncertainty, and sampling bias. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Active learning**, keep query strategies, annotation cost, uncertainty, and sampling bias. as the concrete goal while examining every function below.
...
### `query strategies`
A core part of Active learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`query strategies`**: A core part of Active learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`annotation cost`**: A core part of Active learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`uncertainty`**: A core part of Active learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`sampling bias`**: A core part of Active learning; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Active learning**.
