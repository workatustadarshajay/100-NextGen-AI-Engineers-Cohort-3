...
**Track:** AGENT  
**Companion notebook:** [../Practice/143-agent-security.ipynb](../Practice/143-agent-security.ipynb)

## Purpose

least privilege, sandboxing, injection, confused deputy, and credential isolation.
...
**Agent security** belongs to **Agents and workflows** and is prioritized as **P1**. Its practical scope is least privilege, sandboxing, injection, confused deputy, and credential isolation. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Least Privilege:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Sandboxing:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Injection:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Confused Deputy:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Credential Isolation:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Agent security. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Agent security when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
This lesson focuses on **Agent security**. In plain language: least privilege, sandboxing, injection, confused deputy, and credential isolation. The topic belongs to the **AGENT** track, so interpret every operation through this larger foundation: An agent is a bounded control loop around a model and tools. Reliability comes from strict schemas, least privilege, observable steps, explicit termination, approval gates, and treating model-generated actions as untrusted proposals.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Agent security**, keep least privilege, sandboxing, injection, confused deputy, and credential isolation. as the concrete goal while examining every function below.
...
### `least privilege`
A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`least privilege`**: A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`sandboxing`**: A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`injection`**: A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`confused deputy`**: A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`credential isolation`**: A core part of Agent security; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Agent security**.
