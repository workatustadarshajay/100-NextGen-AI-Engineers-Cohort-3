...
**Track:** DA  
**Companion notebook:** [../Practice/06-statistics-and-hypothesis-testing.ipynb](../Practice/06-statistics-and-hypothesis-testing.ipynb)

## Purpose

Descriptive statistics, confidence intervals, correlation, and two-sample hypothesis tests.
...
This lesson focuses on **Statistics and Hypothesis Testing**. In plain language: Descriptive statistics, confidence intervals, correlation, and two-sample hypothesis tests. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Statistics and Hypothesis Testing**, keep descriptive statistics, confidence intervals, correlation, and two-sample hypothesis tests. as the concrete goal while examining every function below.
...
### `np.mean/median`
Measure central tendency. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `np.var/std`
Measure dispersion. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stats.sem`
Computes standard error of the mean. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stats.ttest_ind`
Tests whether two independent means differ. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stats.t.interval`
Builds a Student-t confidence interval. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`np.mean/median`**: Measure central tendency.
2. **`np.var/std`**: Measure dispersion.
3. **`stats.sem`**: Computes standard error of the mean.
4. **`stats.ttest_ind`**: Tests whether two independent means differ.
5. **`stats.t.interval`**: Builds a Student-t confidence interval.
...
1. Draw the complete state-transition diagram for **Statistics and Hypothesis Testing**.
