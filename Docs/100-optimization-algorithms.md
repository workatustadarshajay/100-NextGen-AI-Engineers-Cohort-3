# 100. Optimization algorithms

**Track:** DL  
**Companion notebook:** [../Practice/100-optimization-algorithms.ipynb](../Practice/100-optimization-algorithms.ipynb)

## Purpose

SGD, momentum, Adam, schedules, weight decay, and gradient clipping.
...
**Optimization algorithms** belongs to **Deep learning** and is prioritized as **P1**. Its practical scope is SGD, momentum, Adam, schedules, weight decay, and gradient clipping. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **SGD:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Momentum:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Adam:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Schedules:** Define it, locate it in the system flow, and identify one measurable behavior.
5. **Weight Decay:** Define it, locate it in the system flow, and identify one measurable behavior.
6. **Gradient Clipping:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Optimization algorithms. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Optimization algorithms when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
### `SGD`
A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`SGD`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`momentum`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`Adam`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`schedules`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
5. **`weight decay`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
6. **`gradient clipping`**: A core part of Optimization algorithms; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Optimization algorithms**.
