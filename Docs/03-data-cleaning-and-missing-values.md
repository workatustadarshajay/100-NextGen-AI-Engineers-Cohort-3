...
**Track:** DA  
**Companion notebook:** [../Practice/03-data-cleaning-and-missing-values.ipynb](../Practice/03-data-cleaning-and-missing-values.ipynb)

## Purpose

Auditing types, duplicates, invalid values, missing data, and robust cleaning rules.
...
This lesson focuses on **Data Cleaning and Missing Values**. In plain language: Auditing types, duplicates, invalid values, missing data, and robust cleaning rules. The topic belongs to the **DA** track, so interpret every operation through this larger foundation: Data analysis converts raw observations into defensible evidence. The analyst must preserve provenance, define the unit of analysis, clean without hiding problems, summarize uncertainty, and distinguish association from causation.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Data Cleaning and Missing Values**, keep auditing types, duplicates, invalid values, missing data, and robust cleaning rules. as the concrete goal while examining every function below.
...
### `isna`
Returns a missing-value mask. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `fillna`
Replaces missing values with an explicit strategy. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `drop_duplicates`
Removes repeated rows. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `pd.to_numeric`
Converts values and can coerce failures to NaN. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `str.strip/title`
Standardizes text fields. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`isna`**: Returns a missing-value mask.
2. **`fillna`**: Replaces missing values with an explicit strategy.
3. **`drop_duplicates`**: Removes repeated rows.
4. **`pd.to_numeric`**: Converts values and can coerce failures to NaN.
5. **`str.strip/title`**: Standardizes text fields.
...
1. Draw the complete state-transition diagram for **Data Cleaning and Missing Values**.
