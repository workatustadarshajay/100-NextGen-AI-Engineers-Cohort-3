...
**Track:** DL  
**Companion notebook:** [../Practice/21-convolutional-neural-networks.ipynb](../Practice/21-convolutional-neural-networks.ipynb)

## Purpose

Image tensors, convolution, kernels, feature maps, padding, stride, pooling, and CNN architecture.
...
### `convolution`
Slides a kernel to detect local patterns. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `kernel/filter`
Small learned matrix used by convolution. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `padding`
Controls spatial size and boundary handling. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stride`
Controls movement between kernel applications. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `max pooling`
Downsamples by retaining local maxima. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`convolution`**: Slides a kernel to detect local patterns.
2. **`kernel/filter`**: Small learned matrix used by convolution.
3. **`padding`**: Controls spatial size and boundary handling.
4. **`stride`**: Controls movement between kernel applications.
5. **`max pooling`**: Downsamples by retaining local maxima.
...
1. Draw the complete state-transition diagram for **Convolutional Neural Networks**.
