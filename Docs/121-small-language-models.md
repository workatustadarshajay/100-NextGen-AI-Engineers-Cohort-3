# 121. Small language models

**Track:** LLM  
**Companion notebook:** [../Practice/121-small-language-models.ipynb](../Practice/121-small-language-models.ipynb)

## Purpose

task selection, distillation, quantization, and edge deployment.
...
**Small language models** belongs to **LLM engineering** and is prioritized as **P2**. Its practical scope is task selection, distillation, quantization, and edge deployment. The lesson treats those phrases as mechanisms to understand and verify, not vocabulary to memorize.

1. **Task Selection:** Define it, locate it in the system flow, and identify one measurable behavior.
2. **Distillation:** Define it, locate it in the system flow, and identify one measurable behavior.
3. **Quantization:** Define it, locate it in the system flow, and identify one measurable behavior.
4. **Edge Deployment:** Define it, locate it in the system flow, and identify one measurable behavior.

**Study sequence:** Begin by drawing the input-to-output flow for Small language models. Next isolate each concept in a tiny example. Compare a baseline with one alternative, deliberately create a failure, and explain the observed evidence. Finally place the mechanism inside a realistic system with versioning, tests, monitoring, security, and a fallback.

**Mastery standard:** A learner has mastered Small language models when they can explain every listed concept without library jargon, implement or simulate the central mechanism, select an appropriate evaluation method, diagnose a counterexample, compare alternatives, and defend a production decision using explicit constraints.
...
### `task selection`
A core part of Small language models; study its mechanism, inputs, outputs, parameters, evidence, and failure modes. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.
...
1. **`task selection`**: A core part of Small language models; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
2. **`distillation`**: A core part of Small language models; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
3. **`quantization`**: A core part of Small language models; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
4. **`edge deployment`**: A core part of Small language models; study its mechanism, inputs, outputs, parameters, evidence, and failure modes.
...
1. Draw the complete state-transition diagram for **Small language models**.
