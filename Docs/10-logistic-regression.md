...
**Track:** ML  
**Companion notebook:** [../Practice/10-logistic-regression.ipynb](../Practice/10-logistic-regression.ipynb)

## Purpose

Binary classification, probabilities, thresholds, regularization, and coefficient interpretation.
...
This lesson focuses on **Logistic Regression**. In plain language: Binary classification, probabilities, thresholds, regularization, and coefficient interpretation. The topic belongs to the **ML** track, so interpret every operation through this larger foundation: Machine learning estimates a reusable mapping from examples. Generalization matters more than memorizing training data, so preprocessing, fitting, selection, and final evaluation must respect strict data boundaries.

The central learning question is not “Which line should I copy?” It is “What information enters, what operation happens, what state is created, what output leaves, and how can I prove that output is useful?” For **Logistic Regression**, keep binary classification, probabilities, thresholds, regularization, and coefficient interpretation. as the concrete goal while examining every function below.
...
### `LogisticRegression`
Models class log-odds as a linear function. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `predict_proba`
Returns class probability estimates. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `stratify`
Preserves class proportions during splitting. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `confusion_matrix`
Counts correct and incorrect predictions by class. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

### `classification_report`
Reports precision, recall, F1, and support. Check accepted input shapes and types, return values, defaults, and fitted attributes before using it in a larger workflow.

## Function-by-function learning sequence

1. **`LogisticRegression`**: Models class log-odds as a linear function.
2. **`predict_proba`**: Returns class probability estimates.
3. **`stratify`**: Preserves class proportions during splitting.
4. **`confusion_matrix`**: Counts correct and incorrect predictions by class.
5. **`classification_report`**: Reports precision, recall, F1, and support.
...
1. Draw the complete state-transition diagram for **Logistic Regression**.
